...
Function ConvertCSV-ToExcel_Tables
{
<#
.SYNOPSIS
...
$worksheet.name = “$((GCI $input).basename.Substring(4))”
...
$len = import-csv $input | Measure-Object

#add table formatting if the csv has at least 1 row
if($len.count -gt 0){
...
$ListObject.TableStyle =   "TableStyleMedium21" # Style Cheat Sheet in French/English: http://msdn.microsoft.com/fr-fr/library/documentformat.openxml.spreadsheet.tablestyle.aspx   
#$ListObject.BoldTopRow = $true
}
...
$tempcsv.close()
   ...
   #provide progress in %  
   $percentInner = ($i/($fileCount))*100
   Write-Progress -Id 1 -Activity Updating -Status 'Progress' -PercentComplete $percentInner -CurrentOperation CSVFiles
