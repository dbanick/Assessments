        ...
        try{ 
            if($cmdType -eq "cmd"){
                if($TrustType -eq "T")
                {
                    ...
                    Invoke-Sqlcmd -inputfile $queryName -Database $database -ServerInstance $instance -ErrorAction Stop -QueryTimeout $QueryTimeout            
                }
            }else{
                Invoke-Sqlcmd2 -inputfile $queryName -Database $database -ServerInstance $instance -ErrorAction Stop -QueryTimeout $QueryTimeout      
            }
        }
        catch{
    ...
    elseif($authtype -eq "SQL"){
        try{
            if($cmdType -eq "cmd"){
                if($TrustType -eq "T")
                {
                    Invoke-Sqlcmd -inputfile $queryName -Database $database -ServerInstance $instance -Username $username -Password $password -ErrorAction Stop -QueryTimeout $QueryTimeout -TrustServerCertificate
                }else {
